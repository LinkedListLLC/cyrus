...
### Changed
- `FORK_DEVELOPMENT.md` records the shell tests in `test/*.test.sh`, what each one covers, and that CI runs them as the **Run Shell Tests** step. Local runs need `GITHUB_APP_ID` and `GITHUB_APP_INSTALLATION_ID` unset. ([CYR-65](https://linear.app/linkedlist/issue/CYR-65))
