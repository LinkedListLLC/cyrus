...
### Added
- The Grok runner catches up with the branch it came from, `gautamjain/cyrus#1`. Every commit that landed there after our import is carried over, minus three: the reverted session-error-reply merge, a `docs/SELF_HOSTING.md` revert that undoes our multi-harness wording, and the deletion of `cross-runner-tool-policy.test.ts` (that test passes here, because this fork has `deriveBuiltInDisallowedTools`). `packages/grok-runner` is now identical to upstream except for `@types/node`. ([CYR-74](https://linear.app/linkedlist/issue/CYR-74), [#53](https://github.com/LinkedListLLC/cyrus/pull/53))
- `packages/grok-runner/src/backend/folderTrust.ts` — writes `~/.grok/trusted_folders.toml` for the session worktree and the main work tree of the repository before `grok agent` starts. Grok drops project MCP names in an untrusted workspace, and headless ACP has no trust prompt. Exported for tests: `collectFolderTrustPaths`, `isPathTrusted`, `parseTrustedFoldersToml`, `resolveGrokHome`, `trustStorePath`. ([CYR-74](https://linear.app/linkedlist/issue/CYR-74), [#53](https://github.com/LinkedListLLC/cyrus/pull/53))
- `packages/grok-runner/src/GrokSkillStager.ts` — stages permitted skills into `<worktree>/.agents/skills/` and removes them when the session ends, the way the Codex stager does. `RunnerConfigBuilder.runnerSupportsManagedSkills()` now answers `true` for `grok`. ([CYR-74](https://linear.app/linkedlist/issue/CYR-74), [#53](https://github.com/LinkedListLLC/cyrus/pull/53))
- `AcpClient` takes `{ timeoutMs, idleTimeoutMs }` per request. `session/prompt` runs with no wall-clock bound and a 15-minute idle watchdog that every notification and reverse RPC resets; control-plane RPCs keep a 60s wall-clock bound, matching the Codex app server. `GROK_TURN_IDLE_TIMEOUT_MS` and `GrokRunnerConfig.turnIdleTimeoutMs` override the default. ([CYR-74](https://linear.app/linkedlist/issue/CYR-74), [#53](https://github.com/LinkedListLLC/cyrus/pull/53))
- `EdgeWorker.resolveRepositoryConfig()` — one helper for the three sites that resolved repository paths inline (constructor, `addNewRepositories`, `updateModifiedRepositories`). A relative `mcpConfigPath` resolves against `repositoryPath`; `~/` and absolute paths keep going through `resolvePath`. ([CYR-74](https://linear.app/linkedlist/issue/CYR-74), [#53](https://github.com/LinkedListLLC/cyrus/pull/53))
- `splitRule` is exported from `cyrus-core`, so `grok-runner/toolPolicy.ts` parses `Name(args)` with the same function the shell matcher uses instead of a copy. ([CYR-74](https://linear.app/linkedlist/issue/CYR-74), [#53](https://github.com/LinkedListLLC/cyrus/pull/53))
...
### Fixed
- `GitHubAppTokenProvider.getAppSlug()` reads `GET /app` with the App JWT and caches the slug for the life of the process; a failed read is not cached, so a transient error does not deny the slug forever. `EdgeWorker.getGitHubBotUsername()` resolves the mention handle from `GITHUB_BOT_USERNAME`, then `GITHUB_APP_SLUG`, then that slug, and `buildAgentContextBlock()` publishes it as `<github_bot_username>`. `resolveGitHubBotUsername()` starts the API read in the background at transport registration. The `verify-and-ship` skill and `glab-mr.md` no longer name `cyrusagent` as a default: with no handle in `<agent_context>`, the tip is left out. The @mention trigger gate still reads `GITHUB_BOT_USERNAME` directly, so reading the slug from the App does not silently narrow which comments wake a deployment that never asked for that filter. ([CYR-75](https://linear.app/linkedlist/issue/CYR-75), [#46](https://github.com/LinkedListLLC/cyrus/pull/46))
- The reviewer on a pull request is the person who delegated the issue, and stays that person. An issue can run more than one session: the delegation, then any number of `@mention` follow-ups. Each session used to resolve the handle from its own creator, so the session that opened the pull request decided the reviewer. A later session now inherits the handle the first session stamped. On JOB-197 the delegated session resolved the correct handle, then stopped after the push without opening a pull request; a follow-up `@mention` started a second session, and that session opened the pull request and requested the commenter.
- The PR-marker hook recognises `gh stack submit`. CYR-60 put the gh-stack extension in the image, and it opens pull requests through a verb the matcher did not know, so the marker and the reviewer request were both skipped. On JOB-197 the marker landed only because the session rewrote the description with `gh pr edit` seconds later. A stack still gets the marker on one pull request only — the one for the branch checked out in the working directory.

### Changed
- `SkillsPluginResolver.buildSkillsGuidance()` takes `{ runnerType }`. A `grok` session is told to read `SKILL.md`; every other runner keeps the Skill tool wording, so the prompt-assembly tests are unchanged. `GrokEventMapper` fills the `system`/`init` message from the live ACP inventory — tools from `available_commands_update._meta.tools`, slash commands from `availableCommands` — and from the staged skill names. ([CYR-74](https://linear.app/linkedlist/issue/CYR-74), [#53](https://github.com/LinkedListLLC/cyrus/pull/53))
- `evaluatePermissionRequest` no longer reads an empty deny list as "no restriction". An allow-list that grants every mutating class plus a scoped `Bash(...)` pattern produces no blanket Bash deny, and `sed -i` used to pass under `Bash(git:*)`. The shell check now runs whenever `hasBashGrant(allow) && !grantsUnrestrictedBash(allow)`. ([CYR-74](https://linear.app/linkedlist/issue/CYR-74), [#53](https://github.com/LinkedListLLC/cyrus/pull/53))
- The Grok runner reads `<worktree>/.env` with `dotenv.parse` instead of a hand-written parser, and one expand map serves both the child environment and `${VAR}` expansion in MCP configuration. Existing process environment still wins over file values. ([CYR-74](https://linear.app/linkedlist/issue/CYR-74), [#53](https://github.com/LinkedListLLC/cyrus/pull/53))
- The **Deployed state** paragraph in `docs/PERSONAS.md` records the `refactorer` routing. All three repositories route it on the `Refactor` label with `allowedTools: "all"`. The paragraph says why the preset is `all` and not `safe`: `safe` withholds `Bash`, and a refactorer that cannot run the tests cannot prove the behaviour did not change. ([CYR-70](https://linear.app/linkedlist/issue/CYR-70))
- `docs/CONFIG_FILE.md` records the per-repository `reviewers` map, which `docs/DOKPLOY.md` documented alone. The entry gives the two identifier forms (`email` or Linear `id`), the `github` handle, and what occurs when the delegating user is absent from the map. ([CYR-66](https://linear.app/linkedlist/issue/CYR-66), [#41](https://github.com/LinkedListLLC/cyrus/pull/41))
...
### Fixed
- The PR-marker hook now reports whether it is registered and whether it runs. It has never been observed doing either: no log line has ever carried `[PrMarkerHook]`, and no pull request Cyrus has opened carries the `<!-- generated-by-cyrus -->` marker the hook exists to add — which also means the GitHub webhook handler cannot recognise those pull requests, so "changes requested" is not being forwarded back. Reviewer routing shares the hook and fails with it, which is how this surfaced. Every link from `EdgeWorker` to the SDK query options was traced and is correct, so the remaining question is empirical rather than structural. Three probes answer it in one delegation: registration, every `PostToolUse(Bash)` invocation logged **before** matching, and the two paths inside `ensureMarker` that previously returned silently when `gh pr view` failed or reported no pull request. The probes are asserted by tests so they cannot be tidied away.
- Reviewer routing now reports what it did on every session, instead of only when a configured map failed to match. The old logging was guarded by "a map exists AND nothing matched", so the two ways of failing — a repository whose `reviewers` map never reached memory, and a webhook carrying no creator — both produced complete silence, indistinguishable from success. Routing was dead in production across several delegations and no log line said so; the absence of the old message was read as evidence that the lookup had matched, which sent the investigation to the wrong layer twice. The line now always states the outcome together with the inputs that produced it (`repo`, number of `reviewers` entries, whether the creator carried an id and an email), so a single log line separates "the map is missing" from "the user is not in it" from "the webhook had nobody". The previous silence was asserted by a test; that assertion is deliberately reversed.
- Commits on the bot's pull requests now carry the bot, not the person the issue was assigned to. The entrypoint already wrote the bot identity into the git config, but the session prompt hands the agent the assignee's GitHub noreply address in its `<assignee>` block, and agents infer from it that they should commit as that person — live runs did exactly that while the config held the correct bot identity. The identity is now also exported as `GIT_AUTHOR_*` / `GIT_COMMITTER_*`, which git reads ahead of configuration, so it outranks both the global file and anything a session sets with `git config` or `git -c`. Unchanged when `GITHUB_APP_SLUG` is unset or the bot lookup fails: nothing is exported, so no identity is ever forced.
- The Docker build no longer breaks on the day xAI ships a new Grok CLI. The image installed the CLI with the installer's default "latest stable" resolution, while `GROK_EXPECTED_VERSION` asserted a fixed version — a pairing that fails every build as soon as the two differ, which is what happened when `0.2.118` replaced the tested `0.2.114` and every Dokploy deploy stopped. The installer has always taken the wanted version as its first argument, contrary to the comment in the Dockerfile claiming it offered no version selector, so the install is now pinned to `GROK_EXPECTED_VERSION` and the assertion is left to catch an installer that ignores it. An empty `GROK_EXPECTED_VERSION` still takes the latest and skips both.
