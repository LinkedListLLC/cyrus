...
- Self-hosted container: the image installs the `gh-stack` extension of the GitHub CLI. Sessions can make, push and merge stacked pull requests with `gh stack`. Read-only sessions cannot use `gh stack`, because the command writes to GitHub. ([CYR-60](https://linear.app/linkedlist/issue/CYR-60))
