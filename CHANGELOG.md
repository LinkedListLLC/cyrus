...
### Added
- New per-repository `reviewOnDelegateInStatus` setting: asking Cyrus to start work on an issue that is already in the `reviewOnStatus` state gives a read-only review of its pull request instead of a normal working session. **In practice you trigger this by @mentioning Cyrus on the issue** — a first-time delegation works too, but when Cyrus built the issue it is already the delegate, and re-delegating an issue it already holds does not start a new session. This is a second route to the same review for workspaces where the Linear application is not subscribed to `Issue` webhooks, and it costs an extra step, since moving the issue no longer triggers the review on its own. Off by default; while it is on, asking Cyrus to work an issue in that state always means "review this" rather than "build this". ([CYR-33](https://linear.app/linkedlist/issue/CYR-33))

### Fixed
- Self-hosted container: sessions now survive a redeploy. Claude keeps each conversation in a directory that was not part of the persisted volume, while Cyrus kept the matching session IDs in the volume — so after every redeploy Cyrus asked for conversations that no longer existed, and each Linear session open at the time could never answer again. The conversation store now lives in the volume with everything else. Your Anthropic credentials come from the environment panel and are not affected. The redeploy that picks this up still restarts the sessions running at that moment; every one after it resumes normally. ([CYR-53](https://linear.app/linkedlist/issue/CYR-53), [#23](https://github.com/LinkedListLLC/cyrus/pull/23))
- A session no longer hangs on "starting task" after you stop it and then send another prompt. Two faults combined here. First, the stop was still on file when the next prompt ran, so Cyrus treated that prompt's outcome as the stop you asked for and posted nothing at all — the Linear panel spun with no result, no error, and no way to tell what had happened. Second, a session that is stopped mid-task can end before Claude saves the conversation, and Cyrus kept asking for that lost conversation on every prompt after it, so the session could never answer again. Cyrus now starts a fresh session with the full issue context when the earlier conversation is gone, tells you it did so, and runs the prompt you sent. ([CYR-53](https://linear.app/linkedlist/issue/CYR-53), [#23](https://github.com/LinkedListLLC/cyrus/pull/23))
- `reviewOnStatus` no longer fails silently when the Linear app cannot receive its trigger. Reviews start from an `Issue` webhook, which Linear only sends if the application is subscribed to the `Issue` resource type — a subscription agent apps often do not have. Without it the setting looked correct and nothing ever happened, with nothing logged to say why. Cyrus now names the requirement at startup, confirms the first time the webhook arrives, and warns once if it sees issue status activity on a channel that cannot start a review. The subscription itself is still a change you make in Linear's settings; see `docs/REVIEW_ON_STATUS.md`. ([CYR-33](https://linear.app/linkedlist/issue/CYR-33)) Sessions with a narrowed tool list (read-only reviewers, Slack chat sessions) were relying on a layer that can be bypassed: Claude Code pre-approves shell commands it judges read-only before Cyrus is asked, and an `allow` rule in a `settings.json` on the machine can quietly override Cyrus's decision entirely. Such a session now also carries an explicit deny list — the write tools plus the shell commands that route around them (`sed -i`, `git commit`, `git push`, `gh api`, `rm`, and similar) — which takes precedence over every other layer. Sessions with an unrestricted tool list are unchanged and can still commit, push and open pull requests; commands a session was explicitly granted are never denied; and read-only reviewers can still post their findings to Linear. ([CYR-25](https://linear.app/linkedlist/issue/CYR-25), [#11](https://github.com/LinkedListLLC/cyrus/pull/11))
- Grok sessions with narrowed shell commands no longer refuse the commands they were granted. A restriction on specific commands was read as a ban on the shell as a whole, so a session allowed only `git pull` could not run anything at all. ([CYR-25](https://linear.app/linkedlist/issue/CYR-25), [#11](https://github.com/LinkedListLLC/cyrus/pull/11))
- Sessions get their tools back. Unless a repository or label explicitly narrowed the tool list, sessions were starting with no tools at all — only file reading and searching — so they could not run commands, edit files, commit, or open a pull request. The built-in default tool list was never reachable: an unconfigured setting collapsed to "an empty list" instead of "not configured", which the resolver read as a deliberate choice to allow nothing. Repositories that do narrow their tool list are unaffected and keep exactly the list they configured. ([CYR-28](https://linear.app/linkedlist/issue/CYR-28), [#10](https://github.com/LinkedListLLC/cyrus/pull/10))
- A repository whose tool list is set to a preset name (`all`, `safe`, `readOnly`, `coordinator`) now expands to that preset. Previously the preset name was taken literally as the name of a single tool, so the session silently ended up with no tools — the same symptom as above, with no error to explain it. Presets already worked when set on a label, and now behave identically on a repository. ([CYR-28](https://linear.app/linkedlist/issue/CYR-28), [#10](https://github.com/LinkedListLLC/cyrus/pull/10))
- Setting `LINEAR_ALLOWED_TOOLS` or `DISALLOWED_TOOLS` to an empty value is now treated as leaving it unset, rather than as a list containing one blank tool name. ([CYR-28](https://linear.app/linkedlist/issue/CYR-28), [#10](https://github.com/LinkedListLLC/cyrus/pull/10))
...
- `reviewOnStatus` reviews now start reliably and stay read-only in cases where they previously did not: the review no longer waits on a webhook that may never arrive (it starts as soon as the review session is created, so a transition can no longer leave a silent empty session behind); a review can read the diff when the sandbox is enabled; the reviewer is no longer told to commit and push its findings when it finishes; and a delegation made while a review is starting can no longer swap the two, which previously left the review with edit and write access. A review that falls back to a local branch because the remote branch is gone now says so instead of presenting it as the pull request. ([CYR-16](https://linear.app/linkedlist/issue/CYR-16), [#2](https://github.com/LinkedListLLC/cyrus/pull/2))
- Enabling the Linear `Issue` webhook subscription (the one `reviewOnStatus` needs) no longer has two costly side effects. Cyrus previously fetched every issue from Linear on *every* state change anywhere in the workspace, even for issues it has nothing to do with; it now settles that from local state first and only fetches when the issue could actually start a review or unblock a waiting session. And moving an issue into the review state and then @mentioning Cyrus on it no longer starts two concurrent reviews of the same pull request — the second is declined with a note on its thread explaining that a review is already underway. ([CYR-35](https://linear.app/linkedlist/issue/CYR-35), [#15](https://github.com/LinkedListLLC/cyrus/pull/15))
...
### Changed
- A Wayfinder session no longer does the work of a ticket that it gave to a second session. A session on a `wayfinder:map` issue found a child ticket, gave that ticket to a new Cyrus session, and then also answered the same ticket itself. Two sessions did one job, and each of them wrote its own answer to the map. A given ticket now belongs to the session that receives it: that session claims the ticket, answers it, closes it, and adds its own line to the map. The session that gave the ticket away keeps its own tickets, tells you which reports it waits for, and ends its turn. It starts again by itself when the report arrives. ([CYR-58](https://linear.app/linkedlist/issue/CYR-58), [#30](https://github.com/LinkedListLLC/cyrus/pull/30))
- Cyrus now uses the latest Opus model by default. The `"opus"` model alias resolves to Opus 5 (`claude-opus-5`, knowledge cutoff May 2026). The default was pinned to an older Opus because the bundled Claude Code runtime did not know about newer models — updating that runtime makes the alias track the latest Opus again. Set a specific model on a repository, label, or issue to override the default. ([CYR-47](https://linear.app/linkedlist/issue/CYR-47), [#21](https://github.com/LinkedListLLC/cyrus/pull/21))
- Updated `@anthropic-ai/claude-agent-sdk` from `0.3.205` to `0.3.220`, which bundles the newer Claude Code runtime that knows about Opus 4.6, 4.7, 4.8, and Opus 5. The Claude Code tool list is unchanged. ([CYR-47](https://linear.app/linkedlist/issue/CYR-47), [#21](https://github.com/LinkedListLLC/cyrus/pull/21))
- Slack chat sessions can refresh a repository before reading it again. Their single allowed command (`git -C * pull`) is now enforced rather than withheld, so it runs and no other shell command does — superseding the CYR-15 entry below, which withheld the shell entirely because the narrowing could not be honoured at the time. ([CYR-20](https://linear.app/linkedlist/issue/CYR-20/enforce-narrowed-bash-grants-on-the-claude-path-via-canusetool), [#9](https://github.com/LinkedListLLC/cyrus/pull/9))
- Slack chat sessions no longer have shell access. Their default configuration allowed a single Bash command (`git -C * pull`), but that narrowing was never enforceable — allowing a command pattern does not forbid the others, so the session effectively had an unrestricted shell. Bash is now withheld from those sessions entirely; they read and search repositories with Read/Grep/Glob instead, and no longer refresh a repository before reading it. ([CYR-15](https://linear.app/linkedlist/issue/CYR-15/claude-allowedtools-does-not-restrict-tools-every-readonly-persona-is), [#8](https://github.com/LinkedListLLC/cyrus/pull/8)) *(superseded by CYR-20 above.)*
...
### Security
- Narrowed shell grants on Claude sessions are now enforced rather than advisory. A grant naming specific commands is checked against every command a call would actually run, so chaining a permitted command with a forbidden one (`git diff HEAD && sed -i …`, `| sh`, `$(…)`, backticks, or a newline) is refused instead of being approved on its first word — matching the behaviour already shipped for Grok sessions. A call whose command is missing or cannot be parsed is refused. Sessions with unrestricted shell access are unaffected. ([CYR-20](https://linear.app/linkedlist/issue/CYR-20/enforce-narrowed-bash-grants-on-the-claude-path-via-canusetool), [#9](https://github.com/LinkedListLLC/cyrus/pull/9))
- Read-only sessions on Claude are now actually read-only. Tool restrictions such as `allowedTools: "readOnly"` were advisory only — Claude's `allowedTools` is an auto-approve list, not a restriction, so every "read-only" research, review, and grilling persona still had `Write`, `Edit`, `Bash`, and `NotebookEdit` available and could modify the repository. Sessions now receive only the tools their configuration actually grants. Linear, Slack, and other MCP tools are unaffected, so read-only reviewers can still report their findings. ([CYR-15](https://linear.app/linkedlist/issue/CYR-15/claude-allowedtools-does-not-restrict-tools-every-readonly-persona-is), [#8](https://github.com/LinkedListLLC/cyrus/pull/8))
