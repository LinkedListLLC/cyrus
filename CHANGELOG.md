...
This section covers the LinkedList fork only. Upstream releases keep their own
sections below.

### Added
- Grok Build is available as an agent runner. Label an issue or name it in a repository's configuration and the session runs on Grok instead of Claude, with the same tool restrictions applied. ([#25](https://github.com/LinkedListLLC/cyrus/pull/25))
- A Grok session gets the same skills as a Claude session. Cyrus copies each permitted skill into the worktree, and tells the session to read `SKILL.md` — Grok has no Skill tool. The session start also reports which skills, tools and slash commands Grok made available. ([CYR-74](https://linear.app/linkedlist/issue/CYR-74), [#53](https://github.com/LinkedListLLC/cyrus/pull/53))
- New per-repository `reviewOnStatus` setting: name a Linear workflow state (for example `"In Review"`) and Cyrus reviews the pull request when an issue moves into it. The review runs as a separate session against a clean checkout of the pull request head, so it is an independent review rather than the author checking its own work, and posts a structured verdict (Blocking / Non-blocking / Nits) back to the issue. It cannot edit files, commit, push or merge. It can only review a branch Cyrus itself created. Omit the setting to keep the previous behaviour. ([CYR-16](https://linear.app/linkedlist/issue/CYR-16), [#26](https://github.com/LinkedListLLC/cyrus/pull/26))
- New `cyrus personas` command. It prints which persona a set of Linear labels resolves to and which tools that persona receives, using the same code a real session uses — so you can check a routing change without delegating an issue. ([CYR-43](https://linear.app/linkedlist/issue/CYR-43), [#27](https://github.com/LinkedListLLC/cyrus/pull/27))
- Two new personas, `wayfinder` and `wayfinder-task`, split on the write boundary: research and grilling sessions get no write tools, task sessions do. ([CYR-37](https://linear.app/linkedlist/issue/CYR-37), [#27](https://github.com/LinkedListLLC/cyrus/pull/27))
- A `Dockerfile`, entrypoint and Dokploy runbook for running Cyrus as a headless self-hosted container. ([#27](https://github.com/LinkedListLLC/cyrus/pull/27))
- Self-hosted container: the image installs the `gh-stack` extension of the GitHub CLI. Sessions can make, push and merge stacked pull requests with `gh stack`. Read-only sessions cannot use `gh stack`, because the command writes to GitHub. ([CYR-60](https://linear.app/linkedlist/issue/CYR-60), [#33](https://github.com/LinkedListLLC/cyrus/pull/33))
- New per-repository `reviewers` setting: map an issue-tracker user to a GitHub handle, and Cyrus requests a review from the person who delegated the issue. A pull request that a bot opens notifies nobody until a review is requested, so this is what tells the delegating user the work is ready. Identify a user by `email` or by `id`, the same way the access-control lists do. A user who is absent from the map only gets a log line: the pull request opens with no reviewer. ([#36](https://github.com/LinkedListLLC/cyrus/pull/36))
- New `cyrus github-token` command. It prints a GitHub token that is valid now, preferring a GitHub App installation token and falling back to `GITHUB_TOKEN`. The self-hosted container uses it for `gh` and for `git push`. ([#36](https://github.com/LinkedListLLC/cyrus/pull/36))
- New `refactorer` persona, for the work that follows a code review. A review can say a method is too complex but cannot change it. Give the issue the `Refactor` label and the next session reads the review, reduces the complexity of only the methods the review named, and proves the behaviour is unchanged by running the tests before and after. It never adds features or fixes bugs on the way through. ([#34](https://github.com/LinkedListLLC/cyrus/pull/34))
- Every session, on every repository, can use the [`refactor-method-complexity-reduce`](https://github.com/github/awesome-copilot) skill, which extracts helper methods out of a method that has grown too complex. The skill states a target method and a complexity threshold; the `refactorer` persona supplies both, because the skill ships with placeholders that nothing fills in automatically. ([#34](https://github.com/LinkedListLLC/cyrus/pull/34))
- Every session, on every repository, can use the [`i-have-adhd`](https://github.com/ayghri/i-have-adhd) skill. It shapes a reply for a reader with ADHD: the next action first, numbered steps, progress restated each turn, and real time estimates. The skill is off until you ask for it. Send `/i-have-adhd` to turn it on, and "stop adhd mode" to turn it off. ([CYR-56](https://linear.app/linkedlist/issue/CYR-56), [#31](https://github.com/LinkedListLLC/cyrus/pull/31))

### Changed
- Self-hosted container: Cyrus can open pull requests under its own GitHub identity. Set `GITHUB_APP_ID` and `GITHUB_APP_INSTALLATION_ID` and put the App private key at `/root/.cyrus/github-app.pem`, and pull requests come from `<app-slug>[bot]` instead of the owner of your personal access token. That owner can then approve the work, which GitHub refuses for a pull request you wrote yourself. The token is minted for each `gh` call and each `git push`, so a session that runs for more than an hour still pushes and opens its pull request. Set `GITHUB_APP_SLUG` as well to attribute the commits to the bot too. With the App variables unset, nothing changes. ([#36](https://github.com/LinkedListLLC/cyrus/pull/36))

- A Wayfinder session no longer does the work of a ticket it gave away. A session on a `wayfinder:map` issue found a child ticket, gave it to a second session, then answered the same ticket itself — two sessions did one job, and each wrote its own line on the map. A ticket now belongs to the session that receives it: that session claims it, answers it, closes it and indexes it. The session that gave it away keeps its own tickets, names the reports it waits for, and ends its turn. It starts again by itself when a report arrives. ([CYR-58](https://linear.app/linkedlist/issue/CYR-58), [#31](https://github.com/LinkedListLLC/cyrus/pull/31))
- Cyrus uses the latest Opus model by default. The `"opus"` alias resolves to Opus 5, instead of the older Opus release the default was pinned to. Set a model on a repository, label or issue to override it. Bundled `@anthropic-ai/claude-agent-sdk` moves to `0.3.220`, the release that knows the model. ([CYR-47](https://linear.app/linkedlist/issue/CYR-47), [#27](https://github.com/LinkedListLLC/cyrus/pull/27))
- `allowedTools: "readOnly"` now means the read-only *code* toolset. It used to resolve to the Slack chat list, which has no `Grep`, no `Glob` and no git inspection — so a read-only `scoper` could not search the code it was scoping. Slack chat sessions are unaffected. ([CYR-37](https://linear.app/linkedlist/issue/CYR-37), [#24](https://github.com/LinkedListLLC/cyrus/pull/24))
- The `builder`, `debugger` and `scoper` prompts are rewritten. The old instruction to never read a file directly and always delegate to a subagent is gone; it fought each repository's own skills and stopped the model reading the files it was editing. ([CYR-37](https://linear.app/linkedlist/issue/CYR-37), [#27](https://github.com/LinkedListLLC/cyrus/pull/27))
- An issue labelled for a read-only `scoper` alongside `Bug` or `Feature` now scopes instead of starting to build. A label saying *how* the session must behave takes precedence over one saying what the work is about. ([#27](https://github.com/LinkedListLLC/cyrus/pull/27))
- `cyrus personas` reports the tools a session actually receives, not just the configured list. Those differ — a grant like `Edit(src/**)` cannot be enforced and is withheld entirely — and the command now shows both, plus a warning for anything dropped. ([#27](https://github.com/LinkedListLLC/cyrus/pull/27))
- A session that dead-ends on a missing conversation is now reported rather than swallowed when no recovery ran for it. ([#27](https://github.com/LinkedListLLC/cyrus/pull/27))

### Fixed
- A long Grok session no longer dies at one hour. The turn now fails only when Grok sends nothing for 15 minutes, so productive work of any length continues. Set `GROK_TURN_IDLE_TIMEOUT_MS` to change that budget, or `0` to remove it. ([CYR-74](https://linear.app/linkedlist/issue/CYR-74), [#53](https://github.com/LinkedListLLC/cyrus/pull/53))
- A Grok session loads the MCP servers of the repository again. Grok refuses project-scoped servers in a workspace it does not trust, and a headless session cannot answer the trust prompt, so Cyrus grants trust for the worktree before it starts Grok. `${VAR}` in an MCP configuration file is also expanded, from the environment of the session and from the `.env` file of the worktree. ([CYR-74](https://linear.app/linkedlist/issue/CYR-74), [#53](https://github.com/LinkedListLLC/cyrus/pull/53))
- Stopping a Grok session no longer posts an error to Linear. A stop request kills the Grok process, and the resulting message was reported as a session failure. ([CYR-74](https://linear.app/linkedlist/issue/CYR-74), [#53](https://github.com/LinkedListLLC/cyrus/pull/53))
- A relative `mcpConfigPath` resolves against the repository, not against the working directory of the Cyrus process. Under systemd that directory is `/`, so `.mcp.json` resolved to `/.mcp.json` and no project MCP server loaded. ([CYR-74](https://linear.app/linkedlist/issue/CYR-74), [#53](https://github.com/LinkedListLLC/cyrus/pull/53))
- A Grok session that holds only scoped shell grants, such as `Bash(git diff:*)`, now refuses a command outside those grants even when nothing is denied by name. ([CYR-74](https://linear.app/linkedlist/issue/CYR-74), [#53](https://github.com/LinkedListLLC/cyrus/pull/53))
- Cyrus downloads the images and files attached to a Linear issue again. The download kept the Linear token it read at start-up, so after the token was refreshed it sent an expired one and every attachment failed with `401 Unauthorized` — while every other Linear operation kept working, because only the downloads held a copy. Cyrus now reads the current token for each file, and after a 401 it refreshes the token one time and downloads the file again. ([CYR-54](https://linear.app/linkedlist/issue/CYR-54), [#47](https://github.com/LinkedListLLC/cyrus/pull/47))
- A pull request that Cyrus opens names the correct handle to @mention. The tip at the end of the description said `@cyrusagent`, an account that belongs to nobody on a self-hosted deployment, so a reader who followed it reached no one. Cyrus now reads the handle from the deployment: `GITHUB_BOT_USERNAME` first, then `GITHUB_APP_SLUG`, then the slug of the GitHub App it runs as. No configuration is necessary. When no handle can be found, the tip is left out instead of naming an account that does not exist. Which comments wake Cyrus is unchanged: only `GITHUB_BOT_USERNAME` limits it to comments that @mention the agent. ([CYR-75](https://linear.app/linkedlist/issue/CYR-75), [#46](https://github.com/LinkedListLLC/cyrus/pull/46))
- Cyrus ignores its own comments on a pull request, whether they carry the bot handle or the `<handle>[bot]` login that a GitHub App writes under. ([CYR-75](https://linear.app/linkedlist/issue/CYR-75), [#46](https://github.com/LinkedListLLC/cyrus/pull/46))
- Sessions get their tools back. Unless a repository or label narrowed the tool list, sessions were starting with no tools at all — no commands, no edits, no pull requests. An unconfigured setting collapsed to "an empty list" instead of "not configured", which the resolver read as a deliberate choice to allow nothing. Repositories that do narrow their list keep exactly what they configured. ([CYR-28](https://linear.app/linkedlist/issue/CYR-28), [#24](https://github.com/LinkedListLLC/cyrus/pull/24))
- A repository whose tool list is a preset name (`all`, `safe`, `readOnly`, `coordinator`) now expands to that preset. The name used to be taken literally as a single tool, so the session silently ended up with nothing. Presets already worked on a label. ([CYR-28](https://linear.app/linkedlist/issue/CYR-28), [#24](https://github.com/LinkedListLLC/cyrus/pull/24))
- Setting `LINEAR_ALLOWED_TOOLS` or `DISALLOWED_TOOLS` to an empty value now means "unset" rather than a list holding one blank name. ([CYR-28](https://linear.app/linkedlist/issue/CYR-28), [#24](https://github.com/LinkedListLLC/cyrus/pull/24))
- Read-only Claude sessions can run the shell commands they were granted. A reviewer allowed `git diff`, `git log` and `gh pr diff` was silently given no shell at all, so it read the files at the tip of a branch with no way to see what had changed — and reported back as if it had. ([CYR-20](https://linear.app/linkedlist/issue/CYR-20), [#24](https://github.com/LinkedListLLC/cyrus/pull/24))
- Grok sessions with narrowed shell commands no longer refuse the commands they were granted. A restriction on specific commands was read as a ban on the shell as a whole. ([CYR-25](https://linear.app/linkedlist/issue/CYR-25), [#25](https://github.com/LinkedListLLC/cyrus/pull/25))
- A refused tool call no longer ends a Grok session silently. The turn continues with the tools the session does hold, and the refusal appears on the Linear timeline. ([#25](https://github.com/LinkedListLLC/cyrus/pull/25))
- One Linear save that changes an issue's state *and* its title no longer loses the state change. Linear sends both in one webhook, and Cyrus handled only the first match — so a rename made while moving an issue stopped a review from starting and left a waiting session parked. ([CYR-46](https://linear.app/linkedlist/issue/CYR-46), [#26](https://github.com/LinkedListLLC/cyrus/pull/26))
- Enabling the Linear `Issue` webhook subscription no longer costs a Linear API call for every state change in the workspace. Cyrus settles from local state first and fetches only when the issue could start a review or unblock a waiting session. ([CYR-35](https://linear.app/linkedlist/issue/CYR-35), [#26](https://github.com/LinkedListLLC/cyrus/pull/26))
- A session no longer hangs on "starting task" after you stop it and send another prompt. The stop stayed on file and was applied to the next prompt, and a session stopped mid-task could end before Claude saved the conversation — after which every later prompt asked for a conversation that no longer existed. Cyrus now starts a fresh session with the full issue context, says so, and runs the prompt you sent. ([CYR-53](https://linear.app/linkedlist/issue/CYR-53), [#27](https://github.com/LinkedListLLC/cyrus/pull/27))
- Self-hosted container: sessions survive a redeploy. Claude kept each conversation outside the persisted volume while Cyrus kept the matching session IDs inside it, so after every redeploy each open Linear session asked for a conversation that was gone. Your credentials come from the environment panel and are unaffected. ([CYR-53](https://linear.app/linkedlist/issue/CYR-53), [#27](https://github.com/LinkedListLLC/cyrus/pull/27))
- An installed Cyrus receives the skills a later release adds. The default skills were copied to `~/.cyrus/cyrus-skills-plugin/` on the first start only, so every new default skill went to new installations and to no other. Cyrus now copies in each default skill it has never sent before, and leaves a skill you deleted deleted. ([CYR-56](https://linear.app/linkedlist/issue/CYR-56), [#31](https://github.com/LinkedListLLC/cyrus/pull/31))
- Self-hosted container: Linear webhooks are accepted behind a reverse proxy. The source-IP allowlist saw the proxy's address rather than Linear's and rejected everything; the image relies on the webhook signature instead. ([#27](https://github.com/LinkedListLLC/cyrus/pull/27))

### Security
- Read-only sessions on Claude are actually read-only. `allowedTools` is an auto-approve list, not a restriction, so every "read-only" research, review and grilling persona still held `Write`, `Edit`, `Bash` and `NotebookEdit` and could change the repository. A session now receives only the tools its configuration grants. Linear, Slack and other MCP tools are unaffected, so a read-only reviewer can still report its findings. ([CYR-15](https://linear.app/linkedlist/issue/CYR-15), [#24](https://github.com/LinkedListLLC/cyrus/pull/24))
- Narrowed shell grants on Claude sessions are enforced rather than advisory. A grant naming specific commands is checked against every command a call would run, so chaining a permitted command with a forbidden one (`git diff HEAD && sed -i …`, `| sh`, `$(…)`, backticks, a newline) is refused instead of approved on its first word. A command that cannot be parsed is refused. Unrestricted sessions are unaffected. ([CYR-20](https://linear.app/linkedlist/issue/CYR-20), [#24](https://github.com/LinkedListLLC/cyrus/pull/24))
- Sessions with a narrowed tool list now also carry an explicit deny list. They were relying on a layer that can be bypassed: Claude Code pre-approves shell commands it judges read-only before Cyrus is asked, and an allow rule in a `settings.json` on the machine can override Cyrus entirely. The deny list — the write tools plus the commands that route around them (`sed -i`, `git commit`, `git push`, `gh api`, `rm` and similar) — takes precedence over every other layer. Commands a session was explicitly granted are never denied. ([CYR-25](https://linear.app/linkedlist/issue/CYR-25), [#24](https://github.com/LinkedListLLC/cyrus/pull/24))
- Grok tool restrictions are enforced. Cyrus launched the CLI in a mode that accepted the permission rules and then approved everything anyway, so a read-only Grok session could write. ([CYR-25](https://linear.app/linkedlist/issue/CYR-25), [#25](https://github.com/LinkedListLLC/cyrus/pull/25))
- A `reviewOnStatus` review stays read-only for its whole life. A delegation made while a review was starting could swap the two and leave the review holding edit and write access; review markers are now bound to the session they were minted for. The reviewer is also no longer told to commit and push its findings when it finishes, and can read the diff when the sandbox is enabled. ([CYR-16](https://linear.app/linkedlist/issue/CYR-16), [#26](https://github.com/LinkedListLLC/cyrus/pull/26))
- A read-only session can no longer write a file through its shell. A grant naming read-only commands still allowed a redirection (`git diff > ~/.bashrc`), a process substitution (`git diff <(rm -rf …)`) and an output flag (`git diff --output=…`) — each of which writes any file on the machine. All three are refused. The shell grants are a strong filter rather than a guarantee, and that is now stated where it matters: enable the sandbox where a session must be provably unable to write. ([#24](https://github.com/LinkedListLLC/cyrus/pull/24))
- A read-only reviewer can no longer merge the change it is reviewing. It was granted the whole Linear tool set, which includes merging a diff, changing an issue's state and deleting comments; it now gets only the tools needed to read the issue and post a review. ([#26](https://github.com/LinkedListLLC/cyrus/pull/26))
- Restricted Grok sessions are no longer unrestricted by accident. Three ways round the restrictions are closed: a tool could declare itself read-only and skip every rule, a restriction Grok has no equivalent for became no restriction at all (and also restored the mode that approves everything), and any tool name containing `__` skipped the checks entirely. A tool Cyrus does not recognise is now refused rather than allowed. ([#25](https://github.com/LinkedListLLC/cyrus/pull/25))
- A refused Grok tool call can no longer be answered with an approval. The option matcher could select "Allow now" when looking for a rejection. ([#25](https://github.com/LinkedListLLC/cyrus/pull/25))
- Self-hosted container: Cyrus refuses to start with direct Linear webhooks enabled and no `LINEAR_WEBHOOK_SECRET`. The signature was previously verified against an empty secret, which anyone can reproduce — so with the source-IP check off (as it is behind a reverse proxy), forged webhooks would have been accepted. ([#27](https://github.com/LinkedListLLC/cyrus/pull/27))
- Self-hosted container: your GitHub token is no longer written in cleartext to the home directory that agent sessions run in. The image's build inputs are also pinned — base image, Claude Code version, and a version assertion on the Grok CLI — so a rebuild cannot silently change the toolchain. ([#27](https://github.com/LinkedListLLC/cyrus/pull/27))
