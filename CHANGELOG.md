...
- New `cyrus github-token` command. It prints a GitHub token that is valid now, preferring a GitHub App installation token and falling back to `GITHUB_TOKEN`. The self-hosted container uses it for `gh` and for `git push`.
