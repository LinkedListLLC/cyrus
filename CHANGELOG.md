...
### Added
- New per-repository `reviewOnDelegateInStatus` setting: delegating an issue to Cyrus while it is already in the `reviewOnStatus` state starts the read-only review instead of a normal working session. This is a second route to the same review for workspaces where the Linear application is not subscribed to `Issue` webhooks — it reacts to a delegation, which is always delivered — and it costs an extra step, since moving the issue no longer triggers the review on its own. Off by default; while it is on, delegating an issue in that state always means "review this" rather than "build this". ([CYR-33](https://linear.app/linkedlist/issue/CYR-33))
