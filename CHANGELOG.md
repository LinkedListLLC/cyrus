...
- A Grok session gets the same skills as a Claude session. Cyrus copies each permitted skill into the worktree, and tells the session to read `SKILL.md` — Grok has no Skill tool. The session start also reports which skills, tools and slash commands Grok made available. ([CYR-74](https://linear.app/linkedlist/issue/CYR-74))
...
### Fixed
- A long Grok session no longer dies at one hour. The turn now fails only when Grok sends nothing for 15 minutes, so productive work of any length continues. Set `GROK_TURN_IDLE_TIMEOUT_MS` to change that budget, or `0` to remove it. ([CYR-74](https://linear.app/linkedlist/issue/CYR-74))
- A Grok session loads the MCP servers of the repository again. Grok refuses project-scoped servers in a workspace it does not trust, and a headless session cannot answer the trust prompt, so Cyrus grants trust for the worktree before it starts Grok. `${VAR}` in an MCP configuration file is also expanded, from the environment of the session and from the `.env` file of the worktree. ([CYR-74](https://linear.app/linkedlist/issue/CYR-74))
- Stopping a Grok session no longer posts an error to Linear. A stop request kills the Grok process, and the resulting message was reported as a session failure. ([CYR-74](https://linear.app/linkedlist/issue/CYR-74))
- A relative `mcpConfigPath` resolves against the repository, not against the working directory of the Cyrus process. Under systemd that directory is `/`, so `.mcp.json` resolved to `/.mcp.json` and no project MCP server loaded. ([CYR-74](https://linear.app/linkedlist/issue/CYR-74))
- A Grok session that holds only scoped shell grants, such as `Bash(git diff:*)`, now refuses a command outside those grants even when nothing is denied by name. ([CYR-74](https://linear.app/linkedlist/issue/CYR-74))
