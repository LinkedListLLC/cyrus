...
- Every session, on every repository, can use the [`i-have-adhd`](https://github.com/ayghri/i-have-adhd) skill. It shapes a reply for a reader with ADHD: the next action first, numbered steps, progress restated each turn, and real time estimates. The skill is off until you ask for it. Send `/i-have-adhd` to turn it on, and "stop adhd mode" to turn it off. ([CYR-56](https://linear.app/linkedlist/issue/CYR-56), [#PR5](https://github.com/LinkedListLLC/cyrus/pull/PR5))

### Changed
- A Wayfinder session no longer does the work of a ticket it gave away. A session on a `wayfinder:map` issue found a child ticket, gave it to a second session, then answered the same ticket itself — two sessions did one job, and each wrote its own line on the map. A ticket now belongs to the session that receives it: that session claims it, answers it, closes it and indexes it. The session that gave it away keeps its own tickets, names the reports it waits for, and ends its turn. It starts again by itself when a report arrives. ([CYR-58](https://linear.app/linkedlist/issue/CYR-58), [#PR5](https://github.com/LinkedListLLC/cyrus/pull/PR5))
