...
### Changed
- A Wayfinder session no longer does the work of a ticket that it gave to a second session. A session on a `wayfinder:map` issue found a child ticket, gave that ticket to a new Cyrus session, and then also answered the same ticket itself. Two sessions did one job, and each of them wrote its own answer to the map. A given ticket now belongs to the session that receives it: that session claims the ticket, answers it, closes it, and adds its own line to the map. The session that gave the ticket away keeps its own tickets, tells you which reports it waits for, and ends its turn. It starts again by itself when the report arrives. ([CYR-58](https://linear.app/linkedlist/issue/CYR-58))
