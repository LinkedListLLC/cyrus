...
### Changed
- Slack chat sessions can refresh a repository before reading it again. Their single allowed command (`git -C * pull`) is now enforced rather than withheld, so it runs and no other shell command does — superseding the CYR-15 entry below, which withheld the shell entirely because the narrowing could not be honoured at the time. ([CYR-20](https://linear.app/linkedlist/issue/CYR-20/enforce-narrowed-bash-grants-on-the-claude-path-via-canusetool))
