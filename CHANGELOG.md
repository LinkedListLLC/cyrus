...
### Added
- New per-repository `reviewOnStatus` setting: name a Linear workflow state (for example `"In Review"`) and Cyrus automatically reviews the pull request when an issue moves into it. The review runs as a separate agent session against a clean checkout of the PR head, so it is an independent review rather than the author checking its own work, and posts a structured verdict (Blocking / Non-blocking / Nits) back to the issue. The review session cannot edit files, commit, push, or merge. Omit the setting to keep the previous behavior. ([CYR-5](https://linear.app/ceedar/issue/CYR-5), [#2](https://github.com/LinkedListLLC/cyrus/pull/2))
